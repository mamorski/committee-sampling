...
func (f *mockRBExp) Gen(sid string, data []byte) (challenge []byte, proof *RBExpProof, err error) {
	// The challenge is built from the session id and the provided data.
	challenge = []byte("challenge_" + sid + "_" + string(data))
	proof = &RBExpProof{
		PiRP:     []byte("fake_piRP"),
		SigmaExp: []byte("fake_sigmaExp"),
		SigmaExa: []byte("fake_sigmaExa"),
	}
	return challenge, proof, nil
}

func (f *mockRBExp) Ver(session string, identityData []byte, auxKey *RBExpAuxKey, _ float64) ([]*RBExpOutput, error) {
