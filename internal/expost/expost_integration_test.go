...
func TestExPostIntegrationFiveNodes(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		mdags[i] = mdag.New(mdagRounds, sessionID, testOracle, nodes[i], syncer, logger, common.ExPostMDAG, "", nil)
	}
	...
	for i := 0; i < nodeCount; i++ {
		...
		exposts[i] = New(nodes[i], mdags[i], sessionID, vk, syncer, expostD, expostBigD, 32, testGradeFunction, logger, nil)
	}
...
func TestExPostIntegrationProverBehavior(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		vk := []byte(fmt.Sprintf("node%d-vk", i))
		exposts[i] = New(nodes[i], mdags[i], sessionID, vk, syncer, expostD, expostBigD, 32, proverGradeFunction, logger, nil)
	}
...
func TestExPostIntegrationConcurrentExecution(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		vk := []byte(fmt.Sprintf("node%d-vk", i))
		exposts[i] = New(nodes[i], mdags[i], sessionID, vk, syncer, expostD, expostBigD, 16, testGradeFunction, logger, nil)
	}
